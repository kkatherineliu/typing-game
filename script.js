...
// change board size variables when screen is resized to avoid generating words outside of the board
window.onresize = () => { 
    boardHeight = 0.8*visualViewport.height;
    boardWidth = 0.75*visualViewport.width;
};
...
function enteredWord(word) {
    if (wordsDisplayed.includes(word)) {
        ...
        // toggle read underline for mistyped words
...
function addWord() {
    let word = wordList[Math.floor(Math.random()*wordList.length)]; // pick a random word from the list
    if (document.getElementById(word) === null) { 

        // create the element to add into the HTML and display on the screen
        ...
        div.innerText = word;
        wordsDisplayed.push(word); // keep track of the words on the screen
    }
}

function slide() {
    // move very word on the screen down
    wordsDisplayed.forEach(w => {
        ...
        if (yCoor < boardHeight) {
            ...
            // make the word red if its nearing the endline as a warning
